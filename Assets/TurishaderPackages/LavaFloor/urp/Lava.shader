...
Shader "UMS/Lava"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			UNITY_INSTANCING_BUFFER_START(UMSLava)
			...
			UNITY_INSTANCING_BUFFER_END(UMSLava)
			...
			float snoise( float2 v )
			{
				...
				return 130.0 * dot( m, g );
					...
					float2 voronoihash47_g46( float2 p )
					{
					...
					float voronoi47_g46( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash47_g46( n + g );
					...
					float2 voronoihash39_g46( float2 p )
					{
					...
					float voronoi39_g46( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash39_g46( n + g );
								o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
								float d = 0.5 * dot( r, r );
						 		if( d<F1 ) {
						 			F2 = F1;
			...
			inline float2 ParallaxOffset( half h, half height, half3 viewDir )
			{
				...
				return h* (v.xy / v.z);
					...
					float2 voronoihash18_g46( float2 p )
					{
					...
					float voronoi18_g46( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash18_g46( n + g );
								o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
								float d = 0.5 * dot( r, r );
						 		if( d<F1 ) {
						 			F2 = F1;
			...
			PackedVaryings VertexFunction( Attributes input  )
			{
				...
				float2 appendResult109 = (float2(ase_positionWS.x , ase_positionWS.z));
				float3 break112 = input.positionOS.xyz;
				float2 appendResult111 = (float2(break112.x , break112.z));
				#if defined( _PROJECTIONMODE_WORLD )
				float2 staticSwitch102 = appendResult109;
				#elif defined( _PROJECTIONMODE_VERTEX )
				float2 staticSwitch102 = appendResult111;
				#else
				float2 staticSwitch102 = appendResult109;
				#endif
				float2 temp_output_66_0_g46 = ( staticSwitch102 * _OverallScale );
				float2 panner82_g46 = ( 1.0 * _Time.y * _Wavescrollspeed + temp_output_66_0_g46);
				float simplePerlin2D79_g46 = snoise( panner82_g46*_Lavawavetiling );
				simplePerlin2D79_g46 = simplePerlin2D79_g46*0.5 + 0.5;
				...
				float3 vertexValue = ( simplePerlin2D79_g46 * input.normalOS * _Lavawaveoffset );
						 ...
						 ) : SV_Target
			{
				...
				float4 _CrustColor_Instance = UNITY_ACCESS_INSTANCED_PROP(UMSLava,_CrustColor);
				float _TurbulenceSpeed_Instance = UNITY_ACCESS_INSTANCED_PROP(UMSLava,_TurbulenceSpeed);
				float mulTime3_g46 = _TimeParameters.x * _TurbulenceSpeed_Instance;
				float time47_g46 = mulTime3_g46;
				float2 voronoiSmoothId47_g46 = 0;
				float2 appendResult109 = (float2(WorldPosition.x , WorldPosition.z));
				float3 break112 = input.ase_texcoord8.xyz;
				float2 appendResult111 = (float2(break112.x , break112.z));
				#if defined( _PROJECTIONMODE_WORLD )
				float2 staticSwitch102 = appendResult109;
				#elif defined( _PROJECTIONMODE_VERTEX )
				...
				#endif
				float2 temp_output_66_0_g46 = ( staticSwitch102 * _OverallScale );
				float2 panner53_g46 = ( 1.0 * _Time.y * _ScrollSpeed + temp_output_66_0_g46);
				float2 panner1_g48 = ( _TimeParameters.x * float2( 0,0 ) + panner53_g46);
				float temp_output_13_0_g48 = 2.0;
				float simplePerlin2D7_g48 = snoise( ( panner1_g48 + float2( 0.01,0 ) )*temp_output_13_0_g48 );
				simplePerlin2D7_g48 = simplePerlin2D7_g48*0.5 + 0.5;
				float simplePerlin2D2_g48 = snoise( panner1_g48*temp_output_13_0_g48 );
				simplePerlin2D2_g48 = simplePerlin2D2_g48*0.5 + 0.5;
				float simplePerlin2D8_g48 = snoise( ( panner1_g48 + float2( 0,0.01 ) )*temp_output_13_0_g48 );
				simplePerlin2D8_g48 = simplePerlin2D8_g48*0.5 + 0.5;
				float4 appendResult9_g48 = (float4(( simplePerlin2D7_g48 - simplePerlin2D2_g48 ) , ( simplePerlin2D8_g48 - simplePerlin2D2_g48 ) , 0.0 , 0.0));
				float2 coords47_g46 = ( ( appendResult9_g48 * 2.0 ) + float4( panner53_g46, 0.0 , 0.0 ) ).xy * 1.0;
				float2 id47_g46 = 0;
				float2 uv47_g46 = 0;
				float fade47_g46 = 0.5;
				float voroi47_g46 = 0;
				float rest47_g46 = 0;
				for( int it47_g46 = 0; it47_g46 <2; it47_g46++ ){
				voroi47_g46 += fade47_g46 * voronoi47_g46( coords47_g46, time47_g46, id47_g46, uv47_g46, 0,voronoiSmoothId47_g46 );
				rest47_g46 += fade47_g46;
				coords47_g46 *= 2;
				fade47_g46 *= 0.5;
				}//Voronoi47_g46
				voroi47_g46 /= rest47_g46;
				float4 ase_positionSSNorm = ScreenPos / ScreenPos.w;
				ase_positionSSNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_positionSSNorm.z : ase_positionSSNorm.z * 0.5 + 0.5;
				float screenDepth50_g46 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_positionSSNorm.xy ),_ZBufferParams);
				float distanceDepth50_g46 = saturate( abs( ( screenDepth50_g46 - LinearEyeDepth( ase_positionSSNorm.z,_ZBufferParams ) ) / ( max( _CrustFadeDistance , 0.0 ) ) ) );
				float lerpResult48_g46 = lerp( 1.0 , voroi47_g46 , pow( distanceDepth50_g46 , 0.1 ));
				float smoothstepResult44_g46 = smoothstep( _LavaLevel , _CrustLevel , lerpResult48_g46);
				float CrustLevel45_g46 = saturate( smoothstepResult44_g46 );
				float4 _LavaColor_Instance = UNITY_ACCESS_INSTANCED_PROP(UMSLava,_LavaColor);
				float time39_g46 = _TimeParameters.x;
				float2 voronoiSmoothId39_g46 = 0;
				...
				float2 paralaxOffset34_g46 = ParallaxOffset( 1 , _LavaParallaxDepth , ase_viewDirTS );
				float2 temp_output_54_0_g46 = ( panner53_g46 + paralaxOffset34_g46 );
				float2 panner1_g49 = ( _TimeParameters.x * float2( 0,0 ) + temp_output_54_0_g46);
				float temp_output_13_0_g49 = 2.0;
				float simplePerlin2D7_g49 = snoise( ( panner1_g49 + float2( 0.01,0 ) )*temp_output_13_0_g49 );
				simplePerlin2D7_g49 = simplePerlin2D7_g49*0.5 + 0.5;
				float simplePerlin2D2_g49 = snoise( panner1_g49*temp_output_13_0_g49 );
				simplePerlin2D2_g49 = simplePerlin2D2_g49*0.5 + 0.5;
				float simplePerlin2D8_g49 = snoise( ( panner1_g49 + float2( 0,0.01 ) )*temp_output_13_0_g49 );
				simplePerlin2D8_g49 = simplePerlin2D8_g49*0.5 + 0.5;
				float4 appendResult9_g49 = (float4(( simplePerlin2D7_g49 - simplePerlin2D2_g49 ) , ( simplePerlin2D8_g49 - simplePerlin2D2_g49 ) , 0.0 , 0.0));
				float2 coords39_g46 = ( ( appendResult9_g49 * 4.0 ) + float4( temp_output_54_0_g46, 0.0 , 0.0 ) ).xy * 1.0;
				float2 id39_g46 = 0;
				float2 uv39_g46 = 0;
				float fade39_g46 = 0.5;
				float voroi39_g46 = 0;
				float rest39_g46 = 0;
				for( int it39_g46 = 0; it39_g46 <5; it39_g46++ ){
				voroi39_g46 += fade39_g46 * voronoi39_g46( coords39_g46, time39_g46, id39_g46, uv39_g46, 0,voronoiSmoothId39_g46 );
				rest39_g46 += fade39_g46;
				coords39_g46 *= 2;
				fade39_g46 *= 0.5;
				}//Voronoi39_g46
				voroi39_g46 /= rest39_g46;
				float4 lerpResult10_g46 = lerp( ( _CrustColor_Instance * ( 1.0 - CrustLevel45_g46 ) * (0.5 + (pow( saturate( ( voroi47_g46 * 10.0 ) ) , 3.0 ) - 0.0) * (1.0 - 0.5) / (1.0 - 0.0)) ) , ( _LavaColor_Instance * voroi39_g46 ) , CrustLevel45_g46);
				
				float3 surf_pos107_g50 = WorldPosition;
				float3 surf_norm107_g50 = WorldNormal;
				float time18_g46 = 0.0;
				float2 voronoiSmoothId18_g46 = 0;
				float2 texCoord3_g47 = input.ase_texcoord9.xy * float2( 1,1 ) + float2( 0,0 );
				float2 panner1_g47 = ( _TimeParameters.x * float2( 0,0 ) + texCoord3_g47);
				float temp_output_13_0_g47 = 30.0;
				float simplePerlin2D7_g47 = snoise( ( panner1_g47 + float2( 0.01,0 ) )*temp_output_13_0_g47 );
				simplePerlin2D7_g47 = simplePerlin2D7_g47*0.5 + 0.5;
				float simplePerlin2D2_g47 = snoise( panner1_g47*temp_output_13_0_g47 );
				simplePerlin2D2_g47 = simplePerlin2D2_g47*0.5 + 0.5;
				float simplePerlin2D8_g47 = snoise( ( panner1_g47 + float2( 0,0.01 ) )*temp_output_13_0_g47 );
				simplePerlin2D8_g47 = simplePerlin2D8_g47*0.5 + 0.5;
				float4 appendResult9_g47 = (float4(( simplePerlin2D7_g47 - simplePerlin2D2_g47 ) , ( simplePerlin2D8_g47 - simplePerlin2D2_g47 ) , 0.0 , 0.0));
				float2 coords18_g46 = ( ( appendResult9_g47 * 0.1 ) + float4( panner53_g46, 0.0 , 0.0 ) ).xy * 3.0;
				float2 id18_g46 = 0;
				float2 uv18_g46 = 0;
				float fade18_g46 = 0.5;
				float voroi18_g46 = 0;
				float rest18_g46 = 0;
				for( int it18_g46 = 0; it18_g46 <2; it18_g46++ ){
				voroi18_g46 += fade18_g46 * voronoi18_g46( coords18_g46, time18_g46, id18_g46, uv18_g46, 0,voronoiSmoothId18_g46 );
				rest18_g46 += fade18_g46;
				coords18_g46 *= 2;
				fade18_g46 *= 0.5;
				}//Voronoi18_g46
				voroi18_g46 /= rest18_g46;
				float temp_output_8_0_g46 = ( voroi18_g46 * pow( ( 1.0 - CrustLevel45_g46 ) , 100.0 ) );
				float height107_g50 = temp_output_8_0_g46;
				float scale107_g50 = 1.0;
				...
				float3 CrustNormal65_g46 = worldToTangentDir42_g50;
				
				float4 LavaEmission7_g46 = ( _LavaColor_Instance * voroi39_g46 * smoothstepResult44_g46 );
				
				float lerpResult42_g46 = lerp( ( pow( temp_output_8_0_g46 , 1.0 ) * _Crustsmoothness ) , _Lavasmoothness , CrustLevel45_g46);
				

				float3 BaseColor = lerpResult10_g46.rgb;
				float3 Normal = CrustNormal65_g46;
				float3 Emission = LavaEmission7_g46.rgb;
				float3 Specular = 0.5;
				float Metallic = 0;
				float Smoothness = lerpResult42_g46;
				float Occlusion = 1;
				float Alpha = 1;
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float2 voronoiSmoothId47_g46 = 0;
				float2 appendResult109 = (float2(WorldPosition.x , WorldPosition.z));
				float3 break112 = input.ase_texcoord4.xyz;
				float2 appendResult111 = (float2(break112.x , break112.z));
				#if defined( _PROJECTIONMODE_WORLD )
				float2 staticSwitch102 = appendResult109;
				#elif defined( _PROJECTIONMODE_VERTEX )
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float2 voronoiSmoothId47_g46 = 0;
				float2 appendResult109 = (float2(WorldPosition.x , WorldPosition.z));
				float3 break112 = input.ase_texcoord2.xyz;
				float2 appendResult111 = (float2(break112.x , break112.z));
				#if defined( _PROJECTIONMODE_WORLD )
				float2 staticSwitch102 = appendResult109;
				#elif defined( _PROJECTIONMODE_VERTEX )
		...
		Pass
		{
						 ...
						 )
			{
				...
				float time18_g46 = 0.0;
				float2 voronoiSmoothId18_g46 = 0;
				float2 texCoord3_g47 = input.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float2 panner1_g47 = ( _TimeParameters.x * float2( 0,0 ) + texCoord3_g47);
				float temp_output_13_0_g47 = 30.0;
				float simplePerlin2D7_g47 = snoise( ( panner1_g47 + float2( 0.01,0 ) )*temp_output_13_0_g47 );
				simplePerlin2D7_g47 = simplePerlin2D7_g47*0.5 + 0.5;
				...
				float4 appendResult9_g47 = (float4(( simplePerlin2D7_g47 - simplePerlin2D2_g47 ) , ( simplePerlin2D8_g47 - simplePerlin2D2_g47 ) , 0.0 , 0.0));
				float2 appendResult109 = (float2(WorldPosition.x , WorldPosition.z));
				float3 break112 = input.ase_texcoord6.xyz;
				float2 appendResult111 = (float2(break112.x , break112.z));
				#if defined( _PROJECTIONMODE_WORLD )
				float2 staticSwitch102 = appendResult109;
				#elif defined( _PROJECTIONMODE_VERTEX )
...
/*ASEBEGIN
Version=19801
Node;AmplifyShaderEditor.PosVertexDataNode;101;-6032,384;Inherit;False;0;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.BreakToComponentsNode;112;-5680,336;Inherit;False;FLOAT3;1;0;FLOAT3;0,0,0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
Node;AmplifyShaderEditor.WorldPosInputsNode;13;-6112,-864;Inherit;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.DynamicAppendNode;111;-5488,352;Inherit;False;FLOAT2;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.DynamicAppendNode;109;-5568,-912;Inherit;False;FLOAT2;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.StaticSwitch;102;-4448,-224;Inherit;False;Property;_ProjectionMode;ProjectionMode;0;0;Create;True;0;0;0;False;0;False;0;0;0;True;;KeywordEnum;2;World;Vertex;Create;True;True;All;9;1;FLOAT2;0,0;False;0;FLOAT2;0,0;False;2;FLOAT2;0,0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT2;0,0;False;6;FLOAT2;0,0;False;7;FLOAT2;0,0;False;8;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.RangedFloatNode;99;-4400,80;Inherit;False;Property;_OverallScale;OverallScale;1;0;Create;True;0;0;0;False;0;False;1;1.5;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;98;-4080,-224;Inherit;False;2;2;0;FLOAT2;0,0;False;1;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.DynamicAppendNode;117;-5568,-800;Inherit;False;FLOAT2;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.DynamicAppendNode;121;-5584,-704;Inherit;False;FLOAT2;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;115;-5232,-624;Inherit;False;2;2;0;FLOAT2;0,0;False;1;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;120;-5232,-528;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;122;-5232,-432;Inherit;False;2;2;0;FLOAT2;0,0;False;1;FLOAT;0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.SimpleAddOpNode;125;-4832,-336;Inherit;False;3;3;0;FLOAT2;0,0;False;1;FLOAT2;0,0;False;2;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.FunctionNode;140;-3600,-160;Inherit;False;LavaBase;2;;46;f88859ea02d1de9418d963f1a1027734;0;1;66;FLOAT2;0,0;False;5;COLOR;69;FLOAT3;68;COLOR;67;FLOAT;0;FLOAT3;75
...
WireConnection;140;66;98;0
WireConnection;1;0;140;69
WireConnection;1;1;140;68
WireConnection;1;2;140;67
WireConnection;1;4;140;0
WireConnection;1;8;140;75
ASEEND*/
//CHKSM=CEB0222005CC6D0DCC475412558947E5FE5F95DC