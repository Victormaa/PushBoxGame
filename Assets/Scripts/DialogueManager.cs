...
public class DialogueManager : MonoBehaviour
{
    ...
    private void Start()
    {
        PrepareBlackScreen();  // 黑幕准备保留
        // ❗不再自动 StartDialogue/PlayDialogue
        // 等待触发器按 F 来调用 TriggerDialogueFromInteract()
    }

    private void Update()
    {
        // ❗不再用 Enter 控制整个对话开始
        // Enter 只用于“播放中”切换到下一句
        if (!isDialoguePlaying) return;

        if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter))
        {
            // PlayDialogue 里每句末尾 WaitUntil 会吃到这个按键
            // 这里不用做额外处理
        }
    }
    ...
    /// ✅ 给“地板触发器 + 按F”调用的入口
    /// </summary>
    public void TriggerDialogueFromInteract(bool allowReplay = false)
    {
        if (isDialoguePlaying) return;

        if (hasFinishedAllLines && !allowReplay)
            return;
    ...
    private IEnumerator FadeInGame(Action onFadeComplete = null)
    {
        ...
        {
            t += Time.unscaledDeltaTime;
            blackScreenCG.alpha = Mathf.Clamp01(1f - (t / fadeDuration));
            yield return null;
        }
